...
void RunHotkeyRuntimeHoldSpecificShiftBit30KeydownActivatesExactSideTest(TestRunMode runMode = TestRunMode::Automated) {
    (void)runMode;

    const std::filesystem::path root = PrepareCaseDirectory("hotkey_runtime_hold_specific_shift_bit30_keydown_activates_exact_side");
    ResetGlobalTestState(root);
    ...
    defaultMode.width = 1280;
    defaultMode.height = 720;

    ModeConfig holdMode;
    holdMode.id = "ShiftHold";
    holdMode.width = 960;
    holdMode.height = 540;

    g_config.modes = { defaultMode, holdMode };
    g_config.defaultMode = defaultMode.id;
    g_config.useSystemKeyRepeat = false;
    g_config.modifiersInterruptKeyRepeat = false;

    HotkeyConfig hotkey;
    hotkey.keys = { VK_RSHIFT, VK_LSHIFT };
    hotkey.secondaryMode = holdMode.id;
    ...
    auto setCurrentModeId = [](const std::string& modeId) {
    ...
    auto getCurrentModeId = []() {
        std::lock_guard<std::mutex> lock(g_modeIdMutex);
        return g_currentModeId;
    };

    setCurrentModeId(defaultMode.id);
    ...
    LPARAM ambiguousLeftShiftKeyDownLParam = BuildTestKeyboardMessageLParam(VK_LSHIFT, true);
    ambiguousLeftShiftKeyDownLParam |= (static_cast<LPARAM>(1) << 30);

    const LRESULT keyDownResult = SubclassedWndProc(window.hwnd(), WM_KEYDOWN, VK_SHIFT, ambiguousLeftShiftKeyDownLParam);
    Expect(keyDownResult == 0, "Expected the hold hotkey to consume the generic VK_SHIFT keydown when blockKeyFromGame is enabled.");
    Expect(capture.messages.empty(),
           "Expected the generic VK_SHIFT keydown not to reach the original WNDPROC when it activates the hold hotkey.");
    Expect(EqualsIgnoreCase(getCurrentModeId(), holdMode.id),
           "Expected a generic VK_SHIFT keydown with the left-Shift scan code and bit30 set to activate the exact LShift hold hotkey while RShift is already held.");
}

void RunHotkeyRuntimeSingleShiftBit30KeydownActivatesExactSideTest(TestRunMode runMode = TestRunMode::Automated) {
    (void)runMode;

    const std::filesystem::path root = PrepareCaseDirectory("hotkey_runtime_single_shift_bit30_keydown_activates_exact_side");
    ResetGlobalTestState(root);
    ...
    HotkeyConfig hotkey;
    hotkey.keys = { VK_LSHIFT };
    hotkey.mainMode = defaultMode.id;
    hotkey.secondaryMode = holdMode.id;
    hotkey.triggerOnHold = true;
    hotkey.blockKeyFromGame = true;
    hotkey.conditions.gameState = { "title" };
    g_config.hotkeys = { hotkey };
    ...
    const LRESULT keyDownResult = SubclassedWndProc(window.hwnd(), WM_KEYDOWN, VK_SHIFT, ambiguousLeftShiftKeyDownLParam);
    Expect(keyDownResult == 0, "Expected the single-key LShift hold hotkey to consume the generic VK_SHIFT keydown when blockKeyFromGame is enabled.");
    Expect(capture.messages.empty(),
           "Expected the single-key LShift hold hotkey not to forward the activating generic VK_SHIFT keydown to the original WNDPROC.");
    Expect(EqualsIgnoreCase(getCurrentModeId(), holdMode.id),
           "Expected pressing LShift after RShift is already held to activate a single-key LShift hold hotkey even when Windows sets bit30 on the generic VK_SHIFT keydown.");
}

    void RunHotkeyRuntimeSingleShiftRepressAfterSpecificShiftReleaseUsesExactTrackerTest(TestRunMode runMode = TestRunMode::Automated) {
        (void)runMode;

        const std::filesystem::path root = PrepareCaseDirectory("hotkey_runtime_single_shift_repress_after_specific_shift_release_uses_exact_tracker");
        ResetGlobalTestState(root);
        ...
        (void)SubclassedWndProc(window.hwnd(), WM_KEYDOWN, VK_SHIFT, BuildTestKeyboardMessageLParam(VK_RSHIFT, true));
        capture.Clear();
        ...
        const LRESULT firstLeftShiftDownResult = SubclassedWndProc(window.hwnd(), WM_KEYDOWN, VK_SHIFT, ambiguousLeftShiftKeyDownLParam);
        Expect(firstLeftShiftDownResult == 0,
            "Expected the first generic VK_SHIFT keydown with the left-Shift scan code to activate the single-key LShift hold hotkey.");
        Expect(capture.messages.empty(),
            "Expected the first activating generic VK_SHIFT keydown not to reach the original WNDPROC.");
        Expect(EqualsIgnoreCase(getCurrentModeId(), holdMode.id),
            "Expected the first LShift press while RShift is held to switch into the hold mode.");

        keyboardState.SetKeyDown(VK_LSHIFT, false);
        keyboardState.Apply();

        const LRESULT leftShiftUpResult = SubclassedWndProc(window.hwnd(), WM_KEYUP, VK_SHIFT,
                                    BuildTestKeyboardMessageLParam(VK_RSHIFT, false));
        Expect(leftShiftUpResult == 0,
            "Expected releasing LShift while RShift remains held to consume the generic VK_SHIFT keyup and revert the hold mode even if the message resolves to the opposite shift side.");
        Expect(capture.messages.empty(),
            "Expected the hold-mode release keyup not to reach the original WNDPROC when blockKeyFromGame is enabled.");
        Expect(EqualsIgnoreCase(getCurrentModeId(), defaultMode.id),
            "Expected releasing LShift while RShift stays held to revert the single-key LShift hold hotkey back to the default mode.");

        keyboardState.SetKeyDown(VK_LSHIFT, true);
        keyboardState.Apply();

        const LRESULT secondLeftShiftDownResult = SubclassedWndProc(window.hwnd(), WM_KEYDOWN, VK_SHIFT, ambiguousLeftShiftKeyDownLParam);
        Expect(secondLeftShiftDownResult == 0,
            "Expected pressing LShift again after releasing it with RShift still held to reactivate the hold hotkey instead of being treated as a repeat.");
        Expect(capture.messages.empty(),
            "Expected the second activating generic VK_SHIFT keydown not to reach the original WNDPROC.");
        Expect(EqualsIgnoreCase(getCurrentModeId(), holdMode.id),
            "Expected the second LShift press while RShift remains held to reactivate the single-key LShift hold hotkey.");
    }

void RunHotkeyRuntimeSpecificShiftWrongKeyupUsesLowLevelExactStateTest(TestRunMode runMode = TestRunMode::Automated) {
    (void)runMode;

    const std::filesystem::path root = PrepareCaseDirectory("hotkey_runtime_specific_shift_wrong_keyup_uses_lowlevel_exact_state");
    ResetGlobalTestState(root);
    ...
    SetLowLevelExactModifierDownForTest(VK_RSHIFT, true);
    QueueLowLevelExactModifierKeyupForTest(VK_LSHIFT);

    const DWORD resolvedVk =
        ResolveTrackedKeyboardVkFromMessageForTest(WM_KEYUP, VK_SHIFT, BuildTestKeyboardMessageLParam(VK_RSHIFT, false));
    Expect(resolvedVk == VK_LSHIFT,
           "Expected a generic VK_SHIFT keyup with the wrong right-Shift scan code to resolve to VK_LSHIFT when the low-level modifier state shows that left Shift was the side that went up, even if the thread keyboard state still reports both shifts down.");
}

void RunHotkeyRuntimeSingleShiftWrongKeyupReleasesFromResolvedVkTest(TestRunMode runMode = TestRunMode::Automated) {
    (void)runMode;

    const std::filesystem::path root = PrepareCaseDirectory("hotkey_runtime_single_shift_wrong_keyup_releases_from_resolved_vk");
    ResetGlobalTestState(root);
    ...
    const LRESULT leftShiftDownResult = SubclassedWndProc(window.hwnd(), WM_KEYDOWN, VK_SHIFT, ambiguousLeftShiftKeyDownLParam);
    Expect(leftShiftDownResult == 0,
           "Expected the single-key LShift hold hotkey to activate before testing the ambiguous release path.");
    Expect(EqualsIgnoreCase(getCurrentModeId(), holdMode.id),
           "Expected the single-key LShift hold hotkey to be active before the ambiguous release path is tested.");

    // Keep the thread keyboard state intentionally ambiguous so the release can only match via
    // the resolved exact message VK, not via live GetKeyState fallback.
    keyboardState.SetKeyDown(VK_LSHIFT, true);
    keyboardState.SetKeyDown(VK_RSHIFT, true);
    keyboardState.Apply();
    ...
    const LRESULT leftShiftUpResult =
        SubclassedWndProc(window.hwnd(), WM_KEYUP, VK_SHIFT, BuildTestKeyboardMessageLParam(VK_RSHIFT, false));
    Expect(leftShiftUpResult == 0,
           "Expected a generic VK_SHIFT keyup with the wrong right-Shift scan code to release the single-key LShift hold hotkey when the exact per-message resolver identifies it as LShift.");
    Expect(capture.messages.empty(),
           "Expected the resolved single-key LShift hold hotkey release not to reach the original WNDPROC when blockKeyFromGame is enabled.");
    Expect(EqualsIgnoreCase(getCurrentModeId(), defaultMode.id),
           "Expected the single-key LShift hold hotkey to revert to the default mode when the exact per-message resolver maps the ambiguous keyup back to LShift.");
}

void RunHotkeyRuntimeHoldSpecificShiftReleaseRevertsOnNonMainKeyupTest(TestRunMode runMode = TestRunMode::Automated) {
    (void)runMode;

    const std::filesystem::path root = PrepareCaseDirectory("hotkey_runtime_hold_specific_shift_release_reverts_on_non_main_keyup");
    ResetGlobalTestState(root);
    ...
    HotkeyConfig hotkey;
    hotkey.keys = { VK_LSHIFT, VK_RSHIFT };
    hotkey.secondaryMode = holdMode.id;
    hotkey.triggerOnHold = true;
    hotkey.blockKeyFromGame = true;
    g_config.hotkeys = { hotkey };
    ...
    const InputHandlerResult keyDownResult =
        HandleHotkeys(window.hwnd(), WM_KEYDOWN, VK_RSHIFT, BuildTestKeyboardMessageLParam(VK_RSHIFT, true), defaultMode.id, "ingame");
    Expect(keyDownResult.consumed, "Expected the hold hotkey keydown to be consumed when blockKeyFromGame is enabled.");
    Expect(EqualsIgnoreCase(getCurrentModeId(), holdMode.id),
           "Expected pressing the RShift main key while LShift is held to activate the hold hotkey mode.");
    ...
    const InputHandlerResult keyUpResult =
        HandleHotkeys(window.hwnd(), WM_KEYUP, VK_LSHIFT, BuildTestKeyboardMessageLParam(VK_LSHIFT, false), holdMode.id, "ingame");
    Expect(!keyUpResult.consumed,
           "Expected releasing the non-main LShift key to revert the hold hotkey without consuming that release event.");
    Expect(EqualsIgnoreCase(getCurrentModeId(), defaultMode.id),
           "Expected releasing LShift while RShift remains held to revert the hold hotkey mode immediately.");
}

    void RunKeyRebindRuntimeSpecificShiftBit30KeydownUsesExactVkTest(TestRunMode runMode = TestRunMode::Automated) {
        DummyWindow window(kWindowWidth, kWindowHeight, runMode == TestRunMode::Visual);
        KeyRebind rebind = MakeEnabledRebind(VK_LSHIFT, 'J');
        PrepareRebindRuntimeCase("key_rebind_runtime_specific_shift_bit30_keydown_uses_exact_vk", { rebind });
        g_gameVersion = GameVersion(1, 13, 0);
        g_showGui.store(false, std::memory_order_release);
        g_config.useSystemKeyRepeat = false;
        PublishConfigSnapshot();
        Expect(window.PumpMessages(), "Expected the test window to stay open before specific-shift bit30 rebind capture setup.");
        ScopedSubclassedInputCapture capture(window.hwnd());
        Expect(window.PumpMessages(), "Expected the test window to stay open after specific-shift bit30 rebind capture setup.");
        capture.Clear();
        ...
        const LRESULT keyDownResult = SubclassedWndProc(window.hwnd(), WM_KEYDOWN, VK_SHIFT, ambiguousLeftShiftKeyDownLParam);
        Expect(keyDownResult == kCapturedWndProcResult,
            "Expected the specific LShift rebind to consume the generic VK_SHIFT keydown and forward the remapped key.");
        Expect(capture.messages.size() == 1,
            "Expected the specific LShift rebind to forward exactly one WM_KEYDOWN for the remapped output when bit30 is set.");
        ExpectCapturedMessage(capture, 0, WM_KEYDOWN, 'J', "Specific LShift bit30 rebind WM_KEYDOWN");

        capture.Clear();
        keyboardState.SetKeyDown(VK_LSHIFT, false);
        keyboardState.SetKeyDown(VK_RSHIFT, true);
        keyboardState.Apply();
        const LRESULT keyUpResult = SubclassedWndProc(window.hwnd(), WM_KEYUP, VK_SHIFT, BuildTestKeyboardMessageLParam(VK_LSHIFT, false));
        Expect(keyUpResult == kCapturedWndProcResult,
            "Expected the specific LShift rebind to consume the matching generic VK_SHIFT keyup and forward the remapped release.");
        Expect(capture.messages.size() == 1,
            "Expected the specific LShift rebind to forward exactly one WM_KEYUP for the remapped output.");
        ExpectCapturedMessage(capture, 0, WM_KEYUP, 'J', "Specific LShift bit30 rebind WM_KEYUP");
    }
